...
void draw_bitmap(unsigned char bitmap[]){
	for(uint16_t y=0;y<lcdHeight;y++){
	    for(uint16_t j=0;j<(lcdWidth>>3);j++){
	        memLcd.writeByteToLineBuffer(j+1, bitmap[j+y*(lcdWidth>>3)]);
	    }
...
int main() {
  ...
  while(1) {
  	//print Raspberry Pi Logo Splash screen
		draw_bitmap(epd_bitmap_rpiLogoPinClipart);
